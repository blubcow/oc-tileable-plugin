...
+function ($) { "use strict";
	...
	TileListWidget.prototype.initUploader = function()
	{
		...
		if(postUrl && (postUrl != '')){
			var uploaderOptions = {
				...
				previewsContainer: this.$el.find('#listUploadContainer').get(0),
				previewTemplate: this.$el.find('#listUploadTemplate').html(),
			...
			// fileupload.js
			this.dropzone.on('addedfile', this.onUploadAddedFile);
	        this.dropzone.on('sending', this.onUploadSending);
	...
	TileListWidget.prototype.onTotalUploadProgress = function(percent) {
		console.log(percent);
		//location.reload(true);
	};
	...
	TileListWidget.prototype.onUploadAddedFile = function(file) {
    ...
    TileListWidget.prototype.onUploadSending = function(file, xhr, formData) {
        //this.addExtraFormData(formData)
    }
    
    TileListWidget.prototype.onUploadSuccess = function(file, response) {
    ...
    TileListWidget.prototype.onUploadError = function(file, error) {
        var $preview = $(file.previewElement)
